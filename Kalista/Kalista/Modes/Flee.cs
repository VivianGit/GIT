...
using Settings = Kalista.Config.Modes.Flee;
using LeagueSharp;
...
namespace Kalista.Modes {
	public class Flee : ModeBase {
		...
		public override bool ShouldBeExecuted() {
			return Config.Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.CustomMode;
		}
		...
		public override void Execute() {
			...
			{
				// Move to the target
				HeroManager.Player.IssueOrder(GameObjectOrder.MoveTo, Target);
			...
			if (Settings.UseAutoAttacks && !Settings.UseWallJumps)
			{
				...
				Config.Orbwalker.ForceTarget(dashObjects.Count > 0 ? dashObjects[0] : null);
			}
			...
			if (Settings.UseWallJumps)
			{
				...
				if (Q.IsReady() && wallCheck != null)
				{
					...
					while (true)
					{
							...
							!checkPoint.ToNavMeshCell().CollFlags.HasFlag(CollisionFlags.Building))
						{
							...
							if (wallCheck != null)
							{
								...
								var sum = Player.GetPath(wallPositionOpposite).Sum(o => o.To2D().LengthSquared());
								if (Math.Sqrt(sum) - Player.Distance(wallPositionOpposite) > 200)
								{
					...
					if (!jumpTriggered)
					{
						Config.Orbwalker.ForceTarget(target);
					}
				}
		...
		private void OnDraw(EventArgs args) {
			...
			{
				var ColorQ = SpellManager.Q.IsReady() ? Color.Red : Color.Teal;
				var CanUseColor = IsJumpPossible ? Color.Green : ColorQ;
				Render.Circle.DrawCircle(FleePosition,50, CanUseColor);
			}
		}
	}
}
