...
namespace Kalista {
	public class VectorHelper {
		...
		public static List<Obj_AI_Base> GetDashObjects(IEnumerable<Obj_AI_Base> predefinedObjectList = null) {
			var objects = predefinedObjectList != null ? predefinedObjectList.ToList() : ObjectManager.Get<Obj_AI_Base>().Where(o => o.IsValidTarget(Orbwalking.GetRealAutoAttackRange(o))).ToList();
			var apexPoint = HeroManager.Player.ServerPosition.To2D() + (HeroManager.Player.ServerPosition.To2D() - Game.CursorPos.To2D()).Normalized() * HeroManager.Player.AttackRange;

			return objects.Where(o => IsLyingInCone(o.ServerPosition.To2D(), apexPoint, HeroManager.Player.ServerPosition.To2D(), Math.PI)).OrderBy(o => o.Distance(apexPoint, true)).ToList();
		}
	}
}
