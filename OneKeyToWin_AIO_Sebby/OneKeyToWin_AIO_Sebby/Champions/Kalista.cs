...
namespace OneKeyToWin_AIO_Sebby
{
    class Kalista
    {
        ...
        {
            Config.SubMenu(Player.ChampionName).SubMenu("Draw").AddItem(new MenuItem("qRange", "Q range", true).SetValue(true));
			...
			Config.SubMenu(Player.ChampionName).SubMenu("Exploit").AddItem(new MenuItem("Exploit", "漏洞", true).SetValue(true));
		}
        ...
        private void Game_OnUpdate(EventArgs args)
        {
			...
			if((Config.Item("Exploit").GetValue<bool>() || 1/Player.AttackDelay<1.7) && Orbwalker.ActiveMode!=Orbwalking.OrbwalkingMode.None)
			{
